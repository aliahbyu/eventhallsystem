...
<nav id="sidebar" class="bg-body-tertiary shadow">
    ...
    <div class="px-0">
        <ul class="list-unstyled components">
            ...
            ?>
            <li class="menu-item">
            <?= $this->Html->link(__('<i class="menu-icon fa-solid fa-house"></i> Home'),
            ['controller' => 'Homes', 'action' => 'index', 'prefix' => false],
            ['class' => 'menu-link', 'escape' => false]) ?>
            </li>

            <li class="menu-item">
            <?= $this->Html->link(__('<i class="menu-icon fa-solid fa-book"></i> Booking'),
            ['controller' => 'Bookings', 'action' => 'add', 'prefix' => false],
            ['class' => 'menu-link', 'escape' => false]) ?>
            </li>
            ...
            <?php if ($this->Identity->isLoggedIn()) { ?>
                ...
                <?php if ($this->Identity->isLoggedIn() && $this->Identity->get('user_group_id') == '1') { ?>
                    ...
                    <li class="menu-item <?= $c_name == 'Adminbookings' ? 'active' : '' ?>">
                        <?= $this->Html->link(
                            __('<i class="menu-icon fa-solid fa-calendar-check"></i> Admin Booking'),
                            ['controller' => 'Adminbookings', 'action' => 'index'],
                            ['class' => 'menu-link', 'escape' => false]
                        ) ?>
                    </li>
                    <li class="menu-item <?= $c_name == 'Halls' ? 'active' : '' ?>">
                    <?= $this->Html->link(
                        __('<i class="menu-icon fa-solid fa-building"></i> Halls'),
                        ['controller' => 'Halls', 'action' => 'index'],
                        ['class' => 'menu-link', 'escape' => false]
                    ) ?>
                    </li>
                    <li class="menu-item <?= $c_name == 'Packages' ? 'active' : '' ?>">
                        <?= $this->Html->link(
                            __('<i class="menu-icon fa-solid fa-box-open"></i> Packages'),
